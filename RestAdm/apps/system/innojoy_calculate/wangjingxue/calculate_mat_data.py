...
def get_inventor_params(base_data_array, file_name):
    ...
    inventor_first_patent_dict = { inventor: np.max(inventor_pantents_bwt_dict[inventor])
                                   for inventor in inventor_pantents_bwt_dict}
    return [inventor_patents_count_dict, inventor_patents_label_dict,
            inventor_first_patent_dict, inventor_patents_rows_dict]


def calculate_inv_perf(inventor_patents_count_dict, inventor_patents_label_dict,
...
def get_key_inventor(base_data_array, file_name):
    ...
    inventor_inv_perf_dict = calculate_inv_perf(inventor_patents_count_dict,
                                                inventor_patents_label_dict, inventor_patents_bwt_dict)
    # 均值和标准差
...
def calculate_key_inventor_hole_label(base_data_array, inventor_patents_rows_dict, key_inventor):
    ...
    if key_inventor_degree == 0:
        ...
        hole_effi = nx.effective_size(key_inventor_network)[key_inventor] /\
                    key_inventor_degree
...
def calculate_depth_breath_knowledge(base_data_array, inventor_patents_rows_dict, key_inventor):
    ...
    pro_list = [(class_number_patent_count + 0.0) /len(inventor_data_rows)
                for class_number_patent_count in class_number_patent_count_list]
...
def get_key_inventor_contacts_list(base_data_array, file_name,
                                   inventor_patents_rows_dict,
                                   key_inventor, window_lenth = 3):
    """
    获取关键研发者中的联系人
    :param base_data_array:
    :param inventor_patents_rows_dict:
    ...
    """
    end_year = utils.get_end_year(file_name)
    key_inventor_partner = get_key_inventor_partner(base_data_array, inventor_patents_rows_dict,
                                                    key_inventor)
    inventor_data_rows = inventor_patents_rows_dict[key_inventor]
...
def calculate_contacts_depth_breath_knowledge(base_data_array, file_name,
                                              inventor_patents_rows_dict,
                                              key_inventor, window_length = 3):
    """
    计算关键研发者联系人知识深度与宽度
    :param base_data_array:
    :param file_name:
    ...
    :return:
    """
    contacts_list = get_key_inventor_contacts_list(base_data_array, file_name,
                                                   inventor_patents_rows_dict,
                                                   key_inventor, window_length)
...
def get_key_inventor_career_age(base_data_array, inventor_patents_rows_dict,
                                key_inventor, inventor_first_year_dict):
    """
...
def calculate_individual_quality(base_data_all, observe_year, inventor):
    ...
    for row_data in base_data_all:
        ...
        if inventor_value:
            inventor_value_list = inventor_value.split(",")
            if inventor in inventor_value_list and \
                    application_date_year <= observe_year:
                individual_quality += 1
    return individual_quality


def calculate_mobility(base_data_array, inventor_patents_rows_dict,
                                key_inventor):
    """
